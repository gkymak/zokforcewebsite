...
:root {
  --zok-primary: #4F76F6;
  --zok-primary-dark: #3D63E8;
  --zok-primary-light: #6B8FF8;
  --zok-bg: #FFFFFF;
  --zok-bg-panel: #F5F7FA;
  --zok-bg-msg: #EDF0F7;
  --zok-bg-user: #4F76F6;
  --zok-text: #1A1A2E;
  --zok-text-muted: #6B7280;
  --zok-border: rgba(79, 118, 246, 0.2);
}
html, body { height: 100%; overflow: hidden; font-family: 'Inter', sans-serif; background: var(--zok-bg); color: var(--zok-text); }
.chat-container { display: flex; flex-direction: column; height: 100%; }
.chat-header {
  background: var(--zok-primary);
  padding: 14px 18px;
...
.chat-avatar {
  ...
  background: #FFFFFF;
  display: flex; align-items: center; justify-content: center; font-size: 17px; font-weight: 800; color: var(--zok-primary); flex-shrink: 0; letter-spacing: -0.5px;
}
.chat-header h3 { font-size: 14px; font-weight: 600; color: white; }
.chat-header p { font-size: 11px; color: rgba(255,255,255,0.7); margin-top: 1px; }
.status-dot {
...
.suggest-btn {
  background: rgba(79,118,246,.08); border: 1px solid var(--zok-border);
  color: var(--zok-primary); padding: 7px 11px; border-radius: 10px;
  font-size: 12.5px; cursor: pointer; text-align: left; font-family: inherit;
  transition: all .2s ease;
}
.suggest-btn:hover { background: rgba(79,118,246,.15); border-color: var(--zok-primary); transform: translateX(3px); }
.input-area {
...
.powered a { color:var(--zok-primary); text-decoration:none; }
</style>
</head>
<body>
